...
OneWire  ds(9);    //A porta do sensor de temperatura
...
void setup(){
  ...
  Serial2.begin(9600);    //Para o envio no APC
...
void readTemp(){    //Função para tratar a temperatura
  ...
  Serial2.write((int)celsius);    //Envia a temperatura como byte
  if(datalog){
...
void readVel(){    //Função para ler a velocidade
  if(b == 1){
    if (millis() - lastRefresh >= REFRESH_TIME)
    {
     ...
     Serial2.write((int)speedk);    //Envio da velocidade como um byte
   if(datalog){
